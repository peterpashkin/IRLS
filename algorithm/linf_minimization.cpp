...
VectorXq Runner::linf_minimization(const VectorXq& b, double_custom epsilon, double_custom M, Graph& g) {
    ...
    while (r.lpNorm<1>() <= 1.0/epsilon) {
        ...
        const double_custom base = 3;
        double_custom exp = 1;

        while(r.lpNorm<1>() <= 1.0/epsilon) {
            VectorXq r_prime = old_r.cwiseProduct(long_step_alpha(alpha_significant, exp));
